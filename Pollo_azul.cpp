...
void Pollo_azul::Habilidad_Pollo(int i)
{
	if (y < 600 - l1) {
		if (dx*bounce > 20 || dx*bounce <-20) {
			...
			if (y + 40 * sin(angulo + (23 * System::Math::PI / 12)) > 600 - l1&& i == 1) {
				y = 600 - l1;
				dx = 0;
			}
			if (y + 40 * sin(angulo + (System::Math::PI / 12)) > 600 - l1&& i == 2) {
				y = 600 - l1;
				dx = 0;
			}
			if (i > 2)
			{
				dx = (40 * cos(angulo + (22 * System::Math::PI / 12)));
				dy = 40 * sin(angulo + (22 * System::Math::PI / 12));
				y += dy;
				x += dx*bounce;
			}
			if (y + 40 * sin(angulo + (22 * System::Math::PI / 12)) > 600 - l1&& i > 2) {
				y = 600 - l1;
				dx = 0;
			}
		}
